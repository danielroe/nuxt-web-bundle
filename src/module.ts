...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup (options, nuxt) {
    ...
    nuxt.hook('nitro:config', config => {
      ...
      config.prerender.routes = [...config.prerender.routes || [], nuxt.options.ssr ? '/' : '/index.html'].filter(i => i !== '/200.html')
    })
    ...
    nuxt.hook('ready', () => {
      nuxt.hook('build:done', async () => {
        ...
        for (const [index, file] of files.sort().reverse().entries()) {
          ...
          const url = new URL(file, options.baseURL).toString().replace(/\/index.html$/, '/').replace(/\.html$/, '')
        ...
        await fsp.writeFile(resolve(nuxt.options.rootDir, 'dist', options.filename), Buffer.from(buf, buf.byteOffset, buf.byteLength))
